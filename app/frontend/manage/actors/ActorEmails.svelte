<script>
  ...
  import { AlertTriangle, MailPlus, Trash2 as Trash } from "lucide-svelte";
...
<div class="flex flex-col gap-3">
  ...
  {#each emails as email}
    ...
    >
      <div class="grow flex flex-col">
        ...
        {#if email.verifiedAt}
          <span class="pr-1.5 label-xs">
            verified <Time timestamp={email.verifiedAt} />
          </span>
        {:else}
          <span class="label-xs pr-1.5">unverified</span>
        {/if}
      </div>
  ...
  <div
    class="text-sm p-1.5 rounded-lg border-2 border-dashed bg-base-300 border-base-100
    flex items-center gap-1.5"
  >
    <label class="input input-xs input-ghost flex items-center gap-3 grow">
      <span class="label-xs opacity-75 whitespace-nowrap">new email</span>
