<script>
  ...
  let create = (credential) => {
    authorize({
      event: prompt.verification("webauthn:create", {
        done: toastAuth("Security key added"),
      }),
      updates: { webauthn_registration: credential },
    });
  };
    ...
    return () => {
      authorize({
        event: prompt.verification("webauthn:delete", {
          ...
          done: toastAuth("Security key removed"),
        }),
        updates: { delete_webauthn_id: credential.id },
        done: updateAuth,
      });
    };
  };
  ...
  let authenticate = (credential) => {
    authorize({
      ...
      updates: { webauthn_verification: credential },
    }).then(updateAuth);
  };
