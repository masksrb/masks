...
<button
  ...
  {type}
  class={props.link
    ? `underline inline ${cls} ${denied ? "animate-denied" : ""}`
    : `btn btn-lg min-w-[130px] ${cls} text-center ${denied ? "animate-denied" : ""}`}
  disabled={isAuthorizing || loading || disabled}
  {onclick}
>
  {#if isAuthorizing || loading}
    <span class="loading loading-spinner loading-md mx-auto opacity-50"></span>
