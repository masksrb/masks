<script>
  ...
  let deleteEmail = () => {
    authorize({
      event: prompt.verification("email:delete", {
        done: () => toast(`Removed ${email.address}`),
        email: email.address,
      }),
      updates: { delete_email_address: email.address },
    });
  };
  ...
  let verifyCode = (code) => {
    authorize({
      event: "email:verify",
      updates: { email_address: email.address, email_code: code },
    });
  };
  ...
  let verifyEmail = () => {
    ...
    authorize({
      event: "email:notify",
      updates: { email_address: email.address },
    });
  };

  let addedEmail = (r) => {
    prompt = r;
    toast(`Added ${newEmail}`);
    newEmail = null;
  };
  ...
  let addEmail = (e) => {
    ...
    authorize({
      ...
      updates: { new_email_address: newEmail },
    });
  };
</script>
