<script>
  ...
  let singleSignOn = (e) => {
    ...
    return authorize({
      event: "sso:request",
      updates: {
        sso_id: sso.provider.id,
        sso_origin: window.location.href,
      },
    });
  };
  ...
  let destroy = (e) => {
    ...
    return authorize({
      event: prompt.verification("sso:delete", {
        provider: sso.provider.name,
        done: () => toast(`Unlinked ${sso.provider.name}`),
      }),
      updates: { delete_sso_id: sso.id },
    });
  };
</script>
