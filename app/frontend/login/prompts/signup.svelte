<script>
  ...
  let validSignup = () => {
    ...
    if (auth.client.requirePassword && !signup.password) {
      return false;
    }

    if (auth.client.requireNickname && !signup.nickname) {
      return false;
    }

    if (auth.client.requireEmail && !signup.email) {
      return false;
    }
...
{:else}
  ...
  <form action="#" {onsubmit}>
    {#if auth.client?.allowNicknames && auth.client?.signupFields?.includes("nickname")}
      <label
        class={[
    ...
    {#if auth.client?.allowEmails && auth.client?.signupFields?.includes("email")}
      <label
        class={[
          "input input-lg input-bordered flex items-center gap-4 w-full mb-3",
          denied ? "input-warning" : "",
    ...
    {#if auth.client?.allowPasswords && auth.client?.signupFields?.includes("password")}
      <PasswordInput
        {auth}
