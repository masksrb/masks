<script>
  ...
  let { showSecrets } = getContext("page");

  let visible = $state(!!showSecrets);
  let toggle = () => {
    visible = !visible;
  };
  ...
  $effect(() => {
    min = auth?.settings?.passwordMinChars;
    max = auth?.settings?.passwordMaxChars;
  ...
  const SvelteComponent = $derived(visible ? Eye : EyeOff);
</script>

<label class={`input input-bordered flex items-center gap-3 w-full ${cls}`}>
  {@render before?.()}

  {#if label}
    <span class="label-text-alt w-[70px]">{label}</span>
  {/if}
  ...
  {#if visible}
    <input
      ...
      class={`text-base min-w-0 grow ${inputClass}`}
      bind:value
      oninput={onChange}
    />
  {:else}
