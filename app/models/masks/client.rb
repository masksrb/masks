...
module Masks
  class Client < ApplicationRecord
    ...
    class << self
      def discover(key = nil, **attrs)
        client = find_by(key: key || attrs.delete(:key))
        client ||= local_records.fetch(key.to_s, nil)

        return unless client

        if !attrs.key?(:internal) || (client.internal? == attrs[:internal])
          client
        end
      end
    ...
    setting :signup_fields, :boolean, default: -> { default_signup_fields }
    ...
    def default_signup_fields
      fields = []
      fields << "nickname" if allow_nicknames?
      fields << "email" if allow_emails?
      fields << "password" if allow_passwords?
      fields
    end
