...
module Masks::Types
  class QueryType < BaseObject
    ...
    def actor(identifier: nil, id: nil)
      actor =
        if identifier
          Masks.actor(identifier, required: true)
        else
    ...
    def clients(**args)
      ...
      if args[:actor]
        actor = Masks.actor(args[:actor], required: true)
    ...
    def providers(**args)
      ...
      if args[:name]
        ...
        scope =
          scope.where(
            "name like ? OR key = ?",
            "#{Masks::Provider.sanitize_sql_like(args[:query])}%",
            args[:query],
          )
      end
    ...
    def tokens(**args)
      ...
      scope = scope.where(client: Masks.client(args[:client])) if args[:client]
