module Masks
  module Modes
    class Server
      ...
      setting :run_workers,
              :boolean,
              env_only: "MASKS_RUN_WORKERS",
              default: true
      setting :skip_migrations,
              :boolean,
              env_only: "MASKS_SKIP_MIGRATIONS",
              default: false
      setting :threads, :int, env_only: "MASKS_THREADS", default: 1
      ...
      setting :db_adapter, :string, env: "MASKS_DB_ADAPTER", key: %i[db adapter]
      ...
      def stats
        {
          ...
          providers: Masks::Provider.count,
      ...
      # after_create :upload_assets, unless: :local
      #
      def local
        @settings["local"]
      end
      ...
      delegate :created_at, :updated_at, to: :installation
      ...
      def favicon_url
        if installation&.favicon&.attached?
          return Masks.storage_url(installation.favicon)
        end
      ...
      def light_logo_url
        if installation&.light_logo&.attached?
          return Masks.storage_url(installation.light_logo)
        end
      ...
      def dark_logo_url
        if installation&.dark_logo&.attached?
          return Masks.storage_url(installation.dark_logo)
        end

        super
      end

      delegate :favicon, :light_logo, :dark_logo, to: :install

      def favicon_file=(file)
        upload_file(:favicon, file)
      end

      def light_logo_file=(file)
        upload_file(:light_logo, file)
      end

      def dark_logo_file=(file)
        upload_file(:dark_logo, file)
      end
          ...
          begin
            adapters =
              (self.settings["adapters"] || {}).deep_merge(
                installation.settings["adapters"] || {},
              )
            adapters
              .map do |k, v|
                next if v["deleted"]
      ...
      def modify_adapter(**args)
        ...
        installation.settings["adapters"] ||= {}
        installation.settings["adapters"][key] ||= {}

        if args[:deleted]
          installation.settings["adapters"][key].clear
          installation.settings["adapters"][key].merge!("deleted" => true)
        else
          installation.settings["adapters"][key].merge!(
      ...
      def upload_file(key, path)
        ...
        install.send(key).attach(io:, filename:)
      end
    end
  end
end
