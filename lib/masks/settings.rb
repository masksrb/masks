  ...
  module Settings
    ...
    class_methods do
      ...
      def setting(name, type, **opts)
        ...
        conf[:attr] = try(:attribute_types)&.keys&.include?(name.to_s)
        conf[:graphql] ||= begin
          cls =
        ...
        existing =
          conf[:defined] || try(:attribute_types)&.keys&.include?(name.to_s)
    ...
    def setting(name)
      ...
      env =
        if conf[:env] || conf[:env_only]
          ENV[conf[:env]&.to_s || conf[:env_only]&.to_s]&.presence
        end
