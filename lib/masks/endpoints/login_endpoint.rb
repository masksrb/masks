module Masks
  class LoginEndpoint
    ...
    def client
      @client ||= Masks.clients.discover(client_id, internal: allow_internal?)
    end
    ...
    def public_json
      json = {
        ...
        trusted: trusted?,
        settled: settled?,
        settings: Masks.conf.public_json,
        scopes: Masks::Scopes.details(scopes),
    ...
    def actor
      if settled?
        @actor ||= Masks.actors.find(settlement["actor"]) if settlement["actor"]
      else
    ...
    def settled!(prompt: "success", error: nil, redirect_uri: nil)
      ...
      if session.structure.bag?(:endpoint)
        session.endpoint["settlement"] = {
          ...
          "actor" => session.actor.current&.id,
        }

        dispatch("login") if !error && session.actor.current
    ...
    def request
      ...
      oauth =
        Masks::Shims::OauthRequest.call(client, oauth_params) do |req|
          ...
          dispatch(Prompt::RESET) if event == Prompt::RESET
