module Avatars
  ...
  class << self
    ...
    def url(actor, style, origin: Current.origin, size: nil)
      ...
      "#{origin}/avatars/#{actor.uuid}/#{style}/#{held}#{query}"
    end

    def urls(actor, origin: Current.origin)
      return nil if origin.blank?

      {
        PHOTO => (url(actor, PHOTO, origin: origin) if held?(actor)),
        IDENTICON => url(actor, IDENTICON, origin: origin),
        INITIALS => url(actor, INITIALS, origin: origin)
      }
    end

    def picture(actor, origin: Current.origin)
      return actor.picture_url if actor.picture_url.present?

      url(actor, held?(actor) ? PHOTO : FALLBACK, origin: origin)
    end
