  ...
  module Mutations
    class IssueProvisioningToken < BaseMutation
      ...
      def resolve(label:, expires_in: nil)
        ...
        token = ::ProvisioningToken.issue!(label: label, by: viewer, expires_in: expires_in)

        audit!(::Event::PROVISIONING_TOKEN_ISSUED, label: token.label, expires_at: token.expires_at.iso8601)
