  ...
  module Mutations
    class ReadSamlMetadata < BaseMutation
      ...
      def resolve(metadata_url: nil, metadata_xml: nil)
        ...
        parsed = ::Federation::Saml.parse_metadata(xml)
      ...
      private
        ...
        def fetch(url)
          probe = ::Provider.new(name: "metadata", metadata_url: url, protocol: ::Provider::SAML)
          probe.validate

          refuse!("metadata URL #{probe.errors[:metadata_url].first}") if probe.errors[:metadata_url].any?

          probe.fetch_text(url, ::Federation::Saml::METADATA_LIMIT)
        end
    end
  end
end
