  ...
  {#if advanced && settings}
    <Alert type="neutral" class="mb-3 -mx-4 rounded-none">
      <div class="flex items-center gap-1.5 mb-1.5 -mt-1.5">
        ...
        >
          {#each settings?.subjectTypes as type}
      ...
      {#if client.subjectType.startsWith("pairwise-")}
        ...
        >
          <p class="label-xs opacity-75">sector</p>
          <input
            type="text"
        ...
        >
          {#snippet before()}
            <p class="label-xs opacity-75">salt</p>
          {/snippet}
        </PasswordInput>
      {/if}
    </Alert>
