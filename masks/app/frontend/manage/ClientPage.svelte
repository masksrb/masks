...
let save = (i) => {
  ...
  let update = mutationStore({
    ...
    query: gql`
        ...
        ${ProviderFragment}
      `,
    variables: { input: { ...(i || changes), id: client.id } },
  });
...
{#key client?.updatedAt}
  <Page {...props} loading={loading || !client}>
    <ClientResult {client} {change} class="mb-3 bg-base-100">
      {#snippet after()}
    ...
    {#if errors}
      <Alert class="mb-3" type="error" {errors}></Alert>
    {/if}
    ...
    {#if client}
      <Tabs
        ...
        props={{ ...props, settings, client, change, save }}
        tabs={{
          general: {
            icon: Cog,
