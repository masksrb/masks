...
let onsubmit = (e) => {
  ...
  authorize({ event: "identify", updates: { identifier: value } }).then((r) => {
    denied = r.auth.warnings.includes("invalid-identifier");
    signup = r.auth.warnings.includes("signup-required");
  });
};
...
{:else if sso}
  <Alert type="info" class="mb-6">
    <div class="flex items-start gap-3">
      <p>
        Your <b>{sso.provider.name}</b> account will be linked once you successfully
        log in...
      </p>
...
<form action="#" {onsubmit}>
  <label
    class={[
      "input input-xl input-bordered flex items-center gap-4 w-full mb-6",
  ...
  >
    ...
    {#if ssoAllowed}
      <PromptContinue
        ...
        {loading}
        disabled={!value}
        class={`-mr-[9px] !min-w-0 ${denied ? "btn-warning" : "btn-primary"}`}
      >
