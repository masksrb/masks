...
let { prompt, authorize, authorizing, loading, setLocked, ...props } = $props();
...
let verifyCode = () => {
  ...
  let updates = { phone_number: phone.number, phone_code: phone.code };
...
let sendCode = (e) => {
  ...
  if (authorizing) {
    updates = { phone_number: phone.number };
  } else {
    updates = { new_phone_number: phone.number };
  }
