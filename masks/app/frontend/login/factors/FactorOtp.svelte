...
let debounceName = _.debounce((id, name) => {
  authorize({ event: "otp:name", updates: { otp_id: id, otp_name: name } });
}, 500);
...
let deleteOtp = (otpSecret) => {
  return () => {
    ...
    authorize({
      ...
      updates: { delete_otp_id: otpSecret.id },
    });
  };
};
...
let debounceAdd = (secret, inputCode) => {
  ...
  return authorize({
    ...
    updates: { new_otp_secret: secret, new_otp_code: inputCode },
  });
};
...
let debounceVerify = _.debounce((secret, inputCode) => {
  ...
  authorize({
    sudo: true,
    event: "otp:verify",
    updates: { otp_id: secret.id, otp_code: inputCode },
  }).then((result) => {
    if (result?.warnings?.length) {
      verifying = false;
      denied = true;
