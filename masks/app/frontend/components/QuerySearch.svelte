...
let search = $state(
  Object.fromEntries(
    [...keys, ...Object.keys(props.toggle || {})].map((k) => {
      if (props.toggle?.[k] && query(k)) {
        return [k, query(k) === "true"];
      }
...
let isEmpty = (u) => {
  return (
    Object.values(
      props.toggle ? _.omit(u, Object.keys(props.toggle)) : u,
...
{#each Object.entries(props?.toggle || {}) as [key, desc]}
  <label class="label cursor-pointer cols-3 mb-2 pl-3">
    <input
      type="checkbox"
      class="toggle toggle-xs"
      checked={query(key) == "true"}
      onclick={(e) => doUpdate(key, query(key) == "true" ? false : true)}
    />

    <span class="grow label-xs"> {desc} </span>
  </label>
{/each}
