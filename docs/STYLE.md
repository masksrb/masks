...
| --- | --- | --- |
| Quickstart, Demo | Tutorial | A path that works end to end on a fresh `./dev` stack. |
| How-to guides | How-to | One task per page, as numbered steps. |
| Key concepts | Explanation | How a part of masks works and why. No steps. |
| Reference | Reference | `manage`, `browser`, `ruby`, and `rails` are generated by `./dev reference`. Never edit them by hand. `specs`, `environment`, `words`, `design`, and `explorer` are written by hand, and `./dev reference` checks `environment` against every variable the code reads. |
...
- Document edge cases: a tenant with no clients, an expired token, a key that
  has been rotated.
...
- Metaphor and poetic nouns: "the shape of it", "a pass over", "carries",
  "signature", "the one record", "rose window" outside the design page.
...
- Links describe their target: "see [registering a client](/guides/register-a-client/)",
