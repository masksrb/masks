...
description: Every environment variable masks reads, and where each one applies.
---
...
| `ENCRYPTION_PRIMARY_KEY` | a development key | Encrypts the columns that hold secrets: signing keys, second-factor secrets, upstream providers' secrets and tokens, adapter credentials, pairwise salts, and setup tokens. |
...
| Variable | Default | |
| --- | --- | --- |
| `MASKS_ATTEMPT_LIMIT` | `10` | Sign-in attempts from one IP address in 3 minutes, including sign-in through an upstream provider. |
...
| `MASKS_RECOVERY_LIMIT` | `5` | Codes and links sent by email or text in 15 minutes, per IP address during sign-in and per account when verifying an address. |
...
| `MASKS_EMAIL_VERIFICATION_LIFETIME` | `172800` (2 days) | How long an email verification link stays valid. |
...
`Masks::Rails` reads no environment variables. Configuration goes in an initializer. The one that
...
`Masks::Client` and `@masks/client` read no environment variables. The issuer and the client's
