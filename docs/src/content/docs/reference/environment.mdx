...
The server refuses to boot in production without these. The error names the missing variable.
...
| `MASKS_SETUP_TOKEN` | none | The token the first-run screen asks for before it creates a tenant's first account. The same token applies to every tenant. |
