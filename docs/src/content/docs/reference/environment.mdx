...
container image's entrypoint can fill in the first four; see [self-hosting](/guides/self-hosting/).

| Variable | Default | |
| --- | --- | --- |
| `SECRET_KEY_BASE` | derived | Signs and encrypts sessions and cookies. Read by Rails. |
| `ENCRYPTION_PRIMARY_KEY` | derived | Encrypts the columns that hold secrets: signing keys, second-factor secrets, providers' secrets, connected accounts' tokens, adapter credentials, pairwise salts, and setup tokens. |
...
:::caution
`SECRET_KEY_BASE` and the three `ENCRYPTION_` values cannot change once data is written. Rows
encrypted under the old keys become unreadable. Keep the master key they're derived from — or each
value you set yourself — with your database backups.
:::
...
Leaving both unset declares Dynamic tenancy instead. See
[Declaring tenants](/guides/core-concepts/#declaring-tenants) for how the three modes differ.
...
The image runs Puma in front of Rails, on `PORT`. Setting the container's command to
`./bin/thrust ./bin/rails server` puts [Thruster](https://github.com/basecamp/thruster) in front of
Puma, on `HTTP_PORT`. See [self-hosting](/guides/self-hosting/#example-composeyml). Background jobs
run inside Puma by default, or in a separate `bin/jobs` process with
`MASKS_JOBS_IN_WEB_SERVER=false`.
...
| Variable | Default | |
| --- | --- | --- |
| `MASKS_THEMES_PATH` | `/rails/themes` | The directory of theme stylesheets, read at boot. See [themes](/guides/themes/). |
