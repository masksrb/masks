...
| [Client engine](#client-engine) | `Masks::Rails` mounted in your application. One variable, and only through the initializer the install generator writes. |
...
| Variable | Default | |
| --- | --- | --- |
| `MASKS_MODE` | `server` | `server` runs masks as an app of its own. `engine` runs it mounted inside another Rails app, with a database, a session, and secrets of its own. `config.masks.mode` sets the same thing. See [Rails apps](/guides/rails/). |
