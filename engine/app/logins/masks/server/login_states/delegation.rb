      ...
      class Delegation < LoginState
        ...
        def undelegated
          ...
          wanted.reject do |_, provider|
            ...
            connection && Masks::Server::Delegation.covering(client: client, actor: actor, connection: connection)
          end
        end
        ...
        private
          ...
          def wanted
            ...
            @wanted ||= begin
              ...
              found = Masks::Server::Provider.delegating.where(key: scopes.map { |scope| Scopes.delegated_provider(scope) }).index_by(&:key)
          ...
          def connection_for(provider)
            ...
            @connections.fetch([ actor.id, provider.id ]) do
              @connections[[ actor.id, provider.id ]] =
                Masks::Server::Connection.live.where(provider: provider, actor: actor).order(connected_at: :desc).find(&:delegable?)
            end
          end
          ...
          def grant!
            wanted.each do |_, provider|
              ...
              Masks::Server::Delegation.grant!(client: client, actor: actor, connection: connection) if connection
            end
          end
          ...
          def finish
            ...
            provider = Masks::Server::Provider.delegating.find_by(id: held["provider_id"])
          ...
          rescue Masks::Server::Provider::Untrusted, Masks::Server::Provider::Refused, Masks::Server::Provider::Unreachable, Masks::Server::Delegation::Refused, ArgumentError => e
          ...
          def connect!(provider, identity, tokens)
            ...
            raise Masks::Server::Provider::Untrusted, "#{provider.name} did not say which account that was" if subject.nil?

            taken = Masks::Server::Connection.live.find_by(provider: provider, subject: subject.to_s)
            mine = Masks::Server::Connection.live.where(provider: provider, actor: actor)

            raise Masks::Server::Delegation::Refused, "that #{provider.name} account is connected to somebody else here" if taken && taken.actor_id != actor.id
            ...
            connection = Masks::Server::Connection.record!(provider: provider, actor: actor, identity: identity.merge("sub" => subject))
