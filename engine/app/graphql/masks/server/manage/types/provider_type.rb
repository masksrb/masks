    ...
    module Manage
      module Types
        class ProviderType < BaseObject
          ...
          def connections
            Masks::Server::Connection.live.where(provider_id: object.id).count
          end
          ...
          def delegations
            Masks::Server::Delegation.live.joins(:connection).where(connections: { provider_id: object.id }).count
          end

          def signed_in
            Masks::Server::Connection.live.where(provider_id: object.id).where.not(signed_in_at: nil).count
          end
        end
      end
    end
